...
def dibujaMunicipiosErroresYELLOW(first,micolor,finrango1, finrango2, fin, ax):
    dibuja= True
    if micolor==1:
      micolor2='yellow'
      milw=2
...
def dibujaMunicipiosYELLOW(first,micolor, ax):
    if micolor==1:
      micolor2='yellow'
      milw=2
    else: 
...
def dibujaMapasNivelMedio():	
	...
	for alfab in range(0,len(shape)):
	 ...
	 if (nombreMunicipio in diccionarioMunicipiosErroneos):
		...
		dibujaMunicipiosErroresYELLOW(shape[alfab],1,finrango1,finrango2,len(shape[alfab].shape.points), ax)
	 else:
		dibujaMunicipiosYELLOW(shape[alfab],1, ax)
	 ax.autoscale_view()
	 ...
	 while(i<len(muni) and encontrado==False):
			if(elimina_tildes(unicode(muni[i].text.upper()))==nombreMunicipioShape.upper()):
				...
				i=i+1    
	 nomFich = "dirTempGeneraMapas/"+nombreMunicipio+"-Medio.png"
	 plt.axis('off')
	...
	# un municipio elegido de forma alfabetica que se coloreara de amarillo
	sf = shapefile.Reader("../Municipios/200001493.shp")
	...
	for alfab in range(0,len(shapes)):
	 ...
	 for shape in sf.shapeRecords():
		...
		if (nombreMunicipio in diccionarioMunicipiosErroneos):
			finrango1= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango1']
			finrango2= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango2']
			dibujaMunicipiosErroresYELLOW(shape,0,finrango1,finrango2,len(shape.shape.points), ax)
		else:
			dibujaMunicipiosYELLOW(shape,0, ax)
	 

	#PARTE DE DIBUJO DE UN MUNICIPIO EN AMARILLO
	 nombreMunicipio = elimina_tildes((shapes[alfab].record[2]).decode('windows-1252'))
	 if (nombreMunicipio in diccionarioMunicipiosErroneos):
		finrango1= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango1']
		finrango2= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango2']
		dibujaMunicipiosErroresYELLOW(shapes[alfab],1,finrango1,finrango2,len(shapes[alfab].shape.points), ax)
	 else:
		dibujaMunicipiosYELLOW(shapes[alfab],1, ax)
	 ...
	 while(i<len(muni) and encontrado==False):
			if(elimina_tildes(unicode(muni[i].text.upper()))==nombreMunicipioShape.upper()):
				...
				i=i+1  
	 nomFich = "dirTempGeneraMapas/"+nombreMunicipio+"-CAM-Medio.png"
	 plt.axis('off')
	 plt.savefig(nomFich, bbox_inches='tight')
	 plt.close(fig)
