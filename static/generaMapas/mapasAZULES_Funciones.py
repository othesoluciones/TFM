...
def dibujaMunicipiosErroresAZUL(first,micolor,finrango1, finrango2, fin, ax):
    dibuja= True
    if micolor==1:
      micolor2='blue'
      milw=2
...
def dibujaMunicipiosAZUL(first,micolor, ax):
    if micolor==1:
      micolor2='blue'
      milw=2
    else: 
...
def dibujaMapasAzules():	
	...
	for alfab in range(0,len(shape)):
	 ...
	 if (nombreMunicipio in diccionarioMunicipiosErroneos):
		...
		dibujaMunicipiosErroresAZUL(shape[alfab],1,finrango1,finrango2,len(shape[alfab].shape.points), ax)
	 else:
		dibujaMunicipiosAZUL(shape[alfab],1, ax)
	 ax.autoscale_view()
	 ...
	 while(i<len(muni) and encontrado==False):
			if(elimina_tildes(unicode(muni[i].text.upper()))==nombreMunicipioShape.upper()):
				...
				i=i+1    
	 nomFich = "dirTempGeneraMapas/"+nombreMunicipio+"-BLUE.png"
	 plt.axis('off')
	...
	# un municipio elegido de forma alfabetica que se coloreara de azul
	sf = shapefile.Reader("../Municipios/200001493.shp")
	...
	for alfab in range(0,len(shapes)):
	 ...
	 #Dibujamos el mapa de la CAM en Gris
	 for shape in sf.shapeRecords():
		nombreMunicipio = elimina_tildes((shape.record[2]).decode('windows-1252'))
		if (nombreMunicipio in diccionarioMunicipiosErroneos):
			finrango1= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango1']
			finrango2= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango2']
			dibujaMunicipiosErroresAZUL(shape,0,finrango1,finrango2,len(shape.shape.points), ax)
		else:
			dibujaMunicipiosAZUL(shape,0, ax)
	 

	#PARTE DE DIBUJO DE UN MUNICIPIO EN AZUL
	 nombreMunicipio = elimina_tildes((shapes[alfab].record[2]).decode('windows-1252'))
	 if (nombreMunicipio in diccionarioMunicipiosErroneos):
		finrango1= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango1']
		finrango2= diccionarioMunicipiosErroneos[nombreMunicipio]['finrango2']
		dibujaMunicipiosErroresAZUL(shapes[alfab],1,finrango1,finrango2,len(shapes[alfab].shape.points), ax)
	 else:
		dibujaMunicipiosAZUL(shapes[alfab],1, ax)
	 ...
	 while(i<len(muni) and encontrado==False):
			if(elimina_tildes(unicode(muni[i].text.upper()))==nombreMunicipioShape.upper()):
				...
				i=i+1  
	 nomFich = "dirTempGeneraMapas/"+nombreMunicipio+"-CAM-BLUE.png"
	 plt.axis('off')
	 plt.savefig(nomFich, bbox_inches='tight')
	 plt.close(fig)
